...
impl MockConnector {
    pub fn new(cur: Currency) -> Self { Self { cur } }
}
...
impl Connector for MockConnector {
    ...
    async fn new_deposit_address(&self) -> Result<Address, GatewayError> {
        Ok(Address { address: format!("mock_{}", Uuid::new_v4()), currency: self.cur })
    }
